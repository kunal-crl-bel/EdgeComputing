...
db = DBManager('./merge_img/', 500)

sim_checker = SimilarityChecker(db, 10)
...
#     # Track frame persistence for new tracks
#     # pending_tracks holds counts for tracks not yet confirmed
#     if 'pending_tracks' not in globals():
#         globals()['pending_tracks'] = {}
#     pending_tracks = globals()['pending_tracks']

#     # Entry / re-entry detection with threshold
#     for tid, cls_id in zip(ids, cls_ids):
#         cls_name = yolo.names[int(cls_id)]
#         if cls_name not in MONITORED_CLASSES:
#             continue
...
#         # If track not yet activated
#         if tid not in active_tracks:
#             # Increment pending count
#             pending_tracks[tid] = pending_tracks.get(tid, 0) + 1
#             # If persisted long enough, confirm entry
#             if pending_tracks[tid] >= total_tracking_frame_count:
#                 # Determine if first time or reappearance
#                 first_seen = first_seen_ts.setdefault(tid, now)
#                 active_tracks[tid] = {
#                     "class": cls_name,
#                     "entry_ts": first_seen,
#                     "last_seen_frame": frame_counter,
...
#                 recent_entries.add(tid)
#                 ts = datetime.fromtimestamp(now).isoformat(timespec="milliseconds")
#                 if pending_tracks[tid] == total_tracking_frame_count and tid not in seen_ids_ever:
#                     # First confirmed entry
#                     seen_ids_ever.add(tid)
#                     logger.info(
#                         f"Entry detected: id=%d class=%s timestamp=%s", int(tid), cls_name, ts
#                     )
#                 else:
#                     # Confirmed re-entry
#                     logger.info(
#                         f"Reappearance detected: id=%d class=%s timestamp=%s", int(tid), cls_name, ts
#                     )
#                 # Remove from pending now that active
#                 del pending_tracks[tid]
#         else:
#             # Active track, reset exit and update last seen
#             active_tracks[tid]["last_seen_frame"] = frame_counter
#             active_tracks[tid]["exit_count"] = 0

#     # Clean up any pending tracks that vanished before threshold
#     for tid in list(pending_tracks.keys()):
#         if tid not in seen_ids_in_frame:
#             del pending_tracks[tid]
...
#                 ts = datetime.fromtimestamp(now).isoformat(timespec="milliseconds")
#                 logger.info(
#                     f"Exit detected: id=%d class=%s timestamp=%s duration=%.3f",
#                     int(tid), info["class"], ts, duration
#                 )
#                 del active_tracks[tid]
...
#                     counts[cls] = counts.get(cls, 0) + 1
#             # Build and send message
#             objects_info = [{"class_label": cls_label, "count": cnt} for cls_label, cnt in counts.items()]
#             msg_bytes = create_detected_object_message(state, objects_info)
#             sock.sendto(msg_bytes, (UDP_IP, UDP_PORT))
#             logger.info("Summary sent: %s", counts)
#             recent_entries.clear()
#         last_summary_time = now
# pending_tracks holds temporary counts & start times for new/re-entering tracks
globals()['pending_tracks'] = {}
from PIL import Image
def process_events(boxes,frame):
    ...
    pending = globals()['pending_tracks']

    # Entry / re-entry detection with threshold & time window
    i = -1
    for tid, cls_id in zip(ids, cls_ids):
        i+=1
        cls_name = yolo.names[int(cls_id)]
        ...
        # If track not yet confirmed active
        if tid not in active_tracks:
            ...
            data = pending.get(tid)
            if data is None or now - data['start_ts'] > REENTRY_WINDOW:
                # start or restart pending counter
                data = {'count': 1, 'start_ts': now}
            else:
                # within re-entry window
                data['count'] += 1

            pending[tid] = data

            # Check if threshold met within window
            if data['count'] >= total_tracking_frame_count and now - data['start_ts'] <= REENTRY_WINDOW:
                first_seen = first_seen_ts.setdefault(tid, data['start_ts'])
                active_tracks[tid] = {
                    "class": cls_name,
                    "entry_ts": first_seen,
                    "last_seen_frame": frame_counter,
                ...
                ts = datetime.fromtimestamp(now).isoformat(timespec="milliseconds")
                if tid not in seen_ids_ever:
                    if not sim_checker.is_new(Image.fromarray(crop)):
                        seen_ids_ever.add(tid)
                ...
                # Clear pending once confirmed
                del pending[tid]
        else:
            # Active track: reset exit count & update last seen
            info = active_tracks[tid]
            info['last_seen_frame'] = frame_counter
            info['exit_count'] = 0

    # Remove stale pending entries not seen or expired
    for tid in list(pending.keys()):
        data = pending[tid]
        if tid not in seen_ids_in_frame or now - data['start_ts'] > REENTRY_WINDOW:
            del pending[tid]
    ...
    if now - last_summary_time >= SUMMARY_INTERVAL:
        if recent_entries:
            ...
            for tid in recent_entries:
                ...
                if cls:
                    counts[cls] = counts.get(cls, 0) + 1
            msg_bytes = create_detected_object_message(state, [
                {"class_label": cls_label, "count": cnt}
                for cls_label, cnt in counts.items()
            ])
            sock.sendto(msg_bytes, (UDP_IP, UDP_PORT))
            logging.info("Summary sent: %s", counts)
            recent_entries.clear()
        last_summary_time = now
...
def reader():
    while True:
        ...
        resized_rgb = cv2.resize(frame, new_size, interpolation=cv2.INTER_LANCZOS4)
...
def inferencer():
    ...
    while True:
        ...
        for frame, res in zip(batch, results):
            ...
            process_events(res.boxes,frame)
        if stop:
            break
    draw_q.put(None)
